...
@main def run_chapter04_option(): Unit =
  ...
  println(KOption.mean(test))

  println(KOption.variance(test))
  println(KOption.variance(Nil))

  println("### 4.3 ###")
  println(KOption.map2(KSome(1), KSome(4))((x: Int, y: Int) => x + y))
  println(KOption.map2(KSome("Hello"), KSome(" World"))((x, y) => x + y))
  println(KOption.map2(KSome("Hello"), KNone)((x, y) => x + y))

  println("### 4.4 ###")
  println(KOption.sequence(List(KSome(2), KSome(4))))
  println(KOption.sequence(List(KSome(2), KSome(4), KNone)))

  println(KOption.parseInts(List("1", "2", "-11")))
  println(KOption.parseInts(List("1", "2", "-11", "jksdhfg")))

  println("### 4.5 ###")
  val y = List("1", "12", "3", "skojf")
  val y2 = List("1", "12", "3", "88")
  val res = KOption.traverse(y)(KOption.parseInt)
  val res2 = KOption.traverse(y2)(KOption.parseInt)
  println(res)
  println(res2)

  println(KOption.sequence2(List(KSome(1), KSome(2))))
  println(KOption.sequence2(List(KSome(1), KSome(2), KNone)))