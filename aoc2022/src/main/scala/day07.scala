...
object day07 extends App:
  ...
  def walkDir(state: (FileTree, List[String]), in: String): (FileTree, List[String]) = in match
    case "$ cd .."     => (state._1, state._2.drop(1))  // move 1 dir back
    case s"$$ cd $dir" => 
      val path: String = if state._2.isEmpty then dir else state._2.head + dir
  ...
  private val (res1, _): (FileTree, List[String]) = input.foldLeft((Map.empty[String, List[(String, Int)]], Nil))(walkDir)
